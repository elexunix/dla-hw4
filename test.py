...
from hw_code.trainer import NVTrainer as Trainer
...
DEFAULT_CHECKPOINT_PATH = ROOT_PATH / "default-test-folder" / "model-best.pth"
...
def main(config, out_file):
  ...
  model = config.init_obj(config["arch"], module_arch, audio_to_mel_fn=deepcopy(datasets["test"].mel_spectrogramer))
  ...
  with torch.no_grad():
    for batch_num, batch in enumerate(tqdm(dataloaders["test"])):
      ...
      wavs = model.inference(batch["mel"])
